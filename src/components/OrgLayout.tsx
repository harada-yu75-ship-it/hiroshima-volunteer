...
export default function OrgLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-amber-600 text-white px-4 h-14 flex items-center justify-between sticky top-0 z-20">
            ...
            {navItems.map(item => (
              <button
                ...
                className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all border-l-2 ${
                  pathname === item.path
                    ? 'bg-amber-50 text-amber-700 font-bold border-amber-600'
                    : 'text-gray-600 hover:bg-amber-50 hover:text-amber-700 border-transparent'
                }`}
              >
